    ...
    def load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "template": {
                ...
                # Empty = llama-server uses chat template embedded in the GGUF (best for Qwen3.6 MoE etc.).
                # Custom files can break llama.cpp's template autoparser (e.g. hand-built tool_call JSON).
                "selected": os.getenv("CHAT_TEMPLATE", ""),
            },
            # Defaults chosen for stability across Qwen3-family thinking models.
            # Aggressive DRY + frequency/presence penalties caused ~25% of requests to
            # emit EOS as the very first token (predicted_n=1, empty content). Disabled here.
            "sampling": {
                ...
                "dry_multiplier": float(os.getenv("DRY_MULTIPLIER", "0.0")),
                "dry_base": float(os.getenv("DRY_BASE", "1.75")),
                "dry_allowed_length": int(os.getenv("DRY_ALLOWED_LENGTH", "2")),
                "dry_penalty_last_n": int(os.getenv("DRY_PENALTY_LAST_N", "0")),
            },
            "performance": {
...
async def lifespan(app: FastAPI):
    ...
    if config_file.exists():
        with open(config_file) as f:
            ...
            # Custom templates often break newer llama.cpp (OSS: Go $vars; basic: tool JSON vs autoparser).
            tpl = saved_config.get("template") or {}
            sel = tpl.get("selected")
            if sel in ("chat-template-oss.jinja", "chat-template-basic.jinja"):
                saved_config.setdefault("template", {})["selected"] = ""
                logger.warning(
                    "Migrated template selected from %s to embedded GGUF template (llama.cpp compatibility)",
                    sel,
                )
